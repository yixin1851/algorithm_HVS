...
CAlpAPSMPAlgoInterface* CAlpAPSMPAlgoInterface::CreateAPSAlgoInterface(SensorType Sensortype, APSRawType Rawtype, std::string strLogDir, PixelFormatType Pixelformat, int code)
{
	...
	else if (Sensortype == ALP_014BA)
	{
		return new CAlp014BAAPSMPAlgorithm(Sensortype, Rawtype, strLogDir, m_nSiteNumber++, Pixelformat, code);
	}
	else
	{
		return nullptr;
...
CAlpDVSMPAlgoInterface* CAlpDVSMPAlgoInterface::CreateDVSAlgoInterface(SensorType Sensortype, std::string strLogDir, PixelFormatType Pixelformat, int code)
{
	...
	{
		return new CAlp014AADVSMPAlgorithm(Sensortype, strLogDir, m_nSiteNumber++, Pixelformat, code);
	}
	else if (Sensortype == ALP_014BA)
	{
		return new CAlp014BADVSMPAlgorithm(Sensortype, strLogDir, m_nSiteNumber++, Pixelformat, code);
	}
	else
	{
		return nullptr;
