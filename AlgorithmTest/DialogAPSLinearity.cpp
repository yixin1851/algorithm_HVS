...
void CDialogAPSLinearity::Linearity()
{
	...
	else
	{
		...
		if (bRet)
		{
			...
			std::vector<QVector<double>> YData(m_LinearityData.SubFrameLinearityData.size());

			for (uint32_t nIndex = 0; nIndex < ExpTime.size(); nIndex++)
			{
				XData.push_back(ExpTime[nIndex]);
				for (uint32_t nChannel = 0; nChannel < m_LinearityData.SubFrameLinearityData.size(); nChannel++)
				{
