...
void DoubleSlider::paintValueLabel(QPainter* painter)
{
    ...
    QString minValueString = QString::number(int(m_minValue));
    QString maxValueString = QString::number(int(m_maxValue));

    QFontMetrics metrics = painter->fontMetrics();
    int mintextWidth = metrics.size(Qt::TextSingleLine, minValueString).width();
    int maxtextWidth = metrics.size(Qt::TextSingleLine, maxValueString).width();
    ...
    int labelWidth = metrics.size(Qt::TextSingleLine, m_label).width();
    ...
    paintColoredRect(QRect(minPos + 4, 37, maxPos - minPos, 2), Qt::darkGreen, painter);
    ...
    //-----paint Handle
    QColor minColor = (m_state == MinHandle) ? Qt::darkGreen : Qt::darkGray;
    QColor maxColor = (m_state == MaxHandle) ? Qt::darkGreen : Qt::darkGray;
